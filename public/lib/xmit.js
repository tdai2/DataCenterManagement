   ...
   this.send = function()
	...
	this.ajaxhandler=function(datao)
		...
		if ( (datao.xmldoc.readyState==4)&&(datao.errState!=99))
			...
			if(!(datao.xmldoc.status>=100 && datao.xmldoc.status<=505))
			{
				//probably an offline response.. trigger back the original rcvr fn
...
//					code.
			...
			var stat_tmp = -1;
			try{
				stat_tmp = datao.xmldoc.statusText;
			}catch(e){
				stat_tmp = -99;
				}
			...
			datao.onrcv({timeout:false,
							...
							dataraw:datatxt,
							HAPI_STATUS:hapival,
							status:stat_tmp==-99?"":datao.xmldoc.status,
							statustext:stat_tmp==-99?"":datao.xmldoc.statusText
							})
			// For mem leak issues, mark for garbage collection
			// [BrandonB] 1/16/2007
			datao.xmldoc=null;
			...
			if (datao.errState==99)
				...
				//else
				//{
					//commented else because must call recv even on abort in the timeout case..
					var stat_tmp = -1;
					try{
						stat_tmp = datao.xmldoc.statusText;
					}catch(e){
...
xmit.gwgetsetMDR=function(functionname, varname, onrechandler)
{
    xmit.gwgetset(functionname, "libifc_mdr.so.1.0", varname, onrechandler);
}
