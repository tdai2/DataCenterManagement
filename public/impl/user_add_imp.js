...
function doadduser()
	...
	if(isResUsername(userName.value))
	{
		alert(eLang.getString('err',0x05));
		return;
	}	
...
function IPMICMD_GetReservedUsers()
{
	if(top.gUserList!=undefined)
	{
		userList = top.gUserList;
		return;
	}
	xmit.get({url:'/rpc/getreservedusers.asp', onrcv:ReservedUsername, status:''});
}

function ReservedUsername(arg)
{
	userList = [];
	//Add the Reserved Username List
	var getReservedUsername = WEBVAR_JSONVAR_GETRESERVEDUSER.WEBVAR_STRUCTNAME_GETRESERVEDUSER;
	if(WEBVAR_JSONVAR_GETRESERVEDUSER.HAPI_STATUS!=0)
	{
		alert(eLang.getString('common','STR_APP_STR_416'));
		return;
	}else
	{
		for(i=0;i<getReservedUsername.length;i++)
		{
			userList.push(getReservedUsername[i].reservedUsername);
		}
	}
	top.gUserList = userList;
	}
